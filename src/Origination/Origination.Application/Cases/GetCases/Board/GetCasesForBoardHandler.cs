...
public sealed record GetCasesForBoardResult(IReadOnlyList<CaseBoardDto> Cases);
...
public sealed class GetCasesForBoardHandler
{
    ...
    public async Task<GetCasesForBoardResult> Handle(GetCasesForBoardQuery query, CancellationToken cancellationToken = default)
    {
        ...
        return new GetCasesForBoardResult(cases.Select(c => new CaseBoardDto(c.Id, c.BrokerId, c.InquiryNotes, c.FactFind, c.Status, c.CreatedAt)).ToList());
    }
}