...
resource "aws_secretsmanager_secret" "document_sql" {
  name = "document/dev/sql"
}

resource "aws_secretsmanager_secret_version" "document_sql" {
  secret_id = aws_secretsmanager_secret.document_sql.id
  secret_string = join("", [
    "Server=", aws_db_instance.this.address, ",1433;",
    "Database=Document;",
    "User Id=", var.db_username, ";",
